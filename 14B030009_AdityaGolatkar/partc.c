...
bit roundoff = 0;
int adcVal=0, avgVal=0, initVal=0, adcValue = 0,temp=0;
...
void main(void)
{
	...
	*/
	...
	else
	{
			...
			//fweight = avgVal / 0.74;
		  //avgVal = fweight;
		  //avgVal = avgVal*(5000/1024);
		if(temp ==0)
		{
			temp++;
		}
		else{
			adcValue = adcValue*(5000/1024) + 700;
			/*int_to_string(adcValue,weight);
		  LCD_StringWrite(weight,4);*/
		if(adcValue < 3300)
			{
				LCD_CmdWrite(0x80);
				LCD_StringWrite("Range1",6);
			}
			else
			{
				LCD_CmdWrite(0x80);
				LCD_StringWrite("Range2",6);
			}
			adcValue =0;
	}}
