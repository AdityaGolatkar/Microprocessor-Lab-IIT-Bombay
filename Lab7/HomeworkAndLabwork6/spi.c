...
void int_to_string(int val,char arr[4]);
...
void timer0_ISR (void) interrupt 1
{
	...
	else
	{
			...
			fweight = avgVal / 0.74;
		  avgVal = fweight;
			int_to_string(avgVal,weight);
			LCD_CmdWrite(0x80);
			LCD_StringWrite(&weight[0],4);
			LCD_CmdWrite(0x86);
			LCD_StringWrite("mg",2);
